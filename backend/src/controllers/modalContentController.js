...
const getAllModalContent = asyncHandler(async(req, res) => {
    try {
        ...
        console.error('Error fetching modal content:', error.message);
...
const createOrUpdateModalContent = asyncHandler(async(req, res) => {
    ...
    const modalContent = await ModalContent.findOneAndUpdate({ modalType: modalType.toLowerCase() }, {
        modalType: modalType.toLowerCase(),
        title,
        content,
...
const deleteModalContent = asyncHandler(async(req, res) => {
    ...
    const modalContent = await ModalContent.findOneAndUpdate({ modalType: modalType.toLowerCase() }, { isActive: false }, { new: true });
