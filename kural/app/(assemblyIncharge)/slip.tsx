  ...
  const handleBluetoothAllow = async () => {
    try {
      ...
      if (Constants?.appOwnership === 'expo') {
        ...
        try {
          // Use require() inside a try/catch instead of dynamic import() to avoid
          // Metro/Expo on Windows emitting an absolute path to the async-require
          // helper (which can break bundling). This still allows us to handle
          // the native module not being available at runtime (e.g., Expo Go).
          // eslint-disable-next-line @typescript-eslint/no-var-requires
          const mod = require('react-native-bluetooth-state-manager');
