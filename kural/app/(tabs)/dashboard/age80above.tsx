...
export default function Age80AboveScreen() {
  const { t } = useLanguage();
  ...
  const [items, setItems] = useState<Age80Above[]>([]);
  const [allItems, setAllItems] = useState<Age80Above[]>([]);
  const [query, setQuery] = useState('');
  const [stats, setStats] = useState({ male: 0, female: 0, other: 0, total: 0 });

  // Filters modal state
  const [filtersVisible, setFiltersVisible] = useState(false);
  const [minAge, setMinAge] = useState<number>(80);
  ...
  const load = async () => {
    try {
      setLoading(true);
      const res = await voterAPI.getAge80AboveVoters({ q: query, page: 1, limit: 50 });
      
      let arr: Age80Above[] = Array.isArray(res?.data) ? res.data : [];
      ...
      arr = arr.map((doc: any) => {
        if (doc && doc.s && typeof doc.s === 'object') {
          ...
          return { ...doc.s, ...doc } as Age80Above;
        }
        return doc as Age80Above;
      });
      setItems(arr);
      setAllItems(arr);
      ...
      console.log('Age 80+ fetch error:', e?.message || e);
      setItems([]);
  ...
  <TouchableOpacity style={styles.backButton} onPress={() => { try { router.back(); } catch (_) { router.replace('/(tabs)/' as any); } }}>
        ...
        <Text style={styles.headerTitle}>{t('dashboard.age80above')}</Text>
          ...
          <View style={styles.filterCard}>
            ...
            <ScrollView style={{ maxHeight: 420 }}>
              ...
              <View style={{ marginBottom: 8 }}>
                <AgeSlider values={[minAge, maxAge]} onChange={(vals) => { setMinAge(vals[0]); setMaxAge(vals[1]); }} min={80} max={120} />
              </View>
            ...
            <View style={styles.filterFooter}>
              <TouchableOpacity
                ...
                onPress={() => {
                  setMinAge(80); setMaxAge(120); setGenderFilter(new Set());
