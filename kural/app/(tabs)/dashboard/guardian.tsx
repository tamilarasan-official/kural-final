...
export default function GuardianScreen() {
  ...
  const [allItems, setAllItems] = useState<Guardian[]>([]);
  ...
  const load = async () => {
    try {
      setLoading(true);
      const res = await voterAPI.getGuardianVoters({ q: query, page: 1, limit: 50 });
      
      let arr: Guardian[] = Array.isArray(res?.data) ? res.data : [];
      ...
      arr = arr.map((doc: any) => {
        if (doc && doc.s && typeof doc.s === 'object') {
          ...
          return { ...doc.s, ...doc } as Guardian;
        }
        return doc as Guardian;
      });
      setItems(arr);
      setAllItems(arr);
      ...
      console.log('Guardian fetch error:', e?.message || e);
      setItems([]);
  ...
  <TouchableOpacity style={styles.backButton} onPress={() => { try { router.back(); } catch (e) { router.replace('/(tabs)/'); } }}>
        ...
        <Text style={styles.headerTitle}>{t('dashboard.guardian')}</Text>
