...
const getVoterName = (voter: TG): string => {
...
const getVoterID = (voter: TG): string => {
...
const getFatherName = (voter: TG): string => {
...
const getDoorNo = (voter: TG): string | number => {
  if (voter.Door_No) return voter.Door_No;
  if (voter.doornumber) return voter.doornumber;
  return '-';
};

// Helper function to get mobile from either format
const getMobile = (voter: TG): string => {
...
export default function TransgenderScreen() {
  ...
  const [allItems, setAllItems] = useState<TG[]>([]);
  ...
  const load = async () => {
    try {
      setLoading(true);
      const res = await voterAPI.getTransgenderVoters({ q: query, page: 1, limit: 50 });
      
      const arr: TG[] = Array.isArray(res?.data) ? res.data : [];
      setItems(arr);
      setAllItems(arr);
      ...
      console.log('Transgender fetch error:', e?.message || e);
      setItems([]);
  ...
  <TouchableOpacity style={styles.backButton} onPress={() => { try { router.back(); } catch (_) { router.replace('/(tabs)/' as any); } }}>
        ...
        <Text style={styles.headerTitle}>{t('Transgender')}</Text>
